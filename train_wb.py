...
def kfold_train(data_dir, model_dir, args):
    ...
    for i,(train_idx, valid_idx) in enumerate(skf.split(dataset.train_df, dataset.train_df['folder_class'])):
            ...
            if args.LR_scheduler == 'GradualWarmupScheduler' :
                ...
                scheduler = GradualWarmupScheduler(optimizer, multiplier=8, total_epoch=5, after_scheduler=scheduler)
            ...
            for epoch in range(args.epochs): # epoch 
                ...
                with torch.no_grad(): # 1 epoch train 끝나고
                    ...
                    print(
                        ...
                        f"[Val] f1 : {val_f1:4.2%} || "
                        f"best f1 : {best_val_f1:4.4%} || "
                        f"loss: {val_loss:4.4}, best loss: {best_val_loss:4.4}"
                    )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lr', type=float, default=0.00009, help='learning rate (default: 1e-3)')
